...
class VeyraOptionsFlow(OptionsFlow):
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        if user_input is not None:
            # Keep unrelated/class options already stored by select entities, but
            # remove obsolete 0.2.x flood-mute options on the next save.
            options = {
                key: value
                for key, value in dict(self.config_entry.options).items()
                if not str(key).startswith("flood_mute_")
            }
